...
run_sql_commands() {
    local sql_user="root"
    local sql_password="root"

    echo -e "\n[INFO] Using SQL credentials: user='$sql_user' password='root'"

    # Vérification des identifiants
    if ! mysql -u "$sql_user" -p"$sql_password" -e ";" &>/dev/null; then
        echo -e "\n[ERROR] Invalid SQL credentials. Make sure the MySQL root user is accessible without a password and this script runs as root."
        exit 1
    fi
